...
- Tag-only OSM publishing preserves way node references and relation members exactly as returned by the live OSM API. The server aborts the update before the network `PUT` if those structural children are unavailable or change during serialization.
- Historical changesets can be checked for relation-member changes with `npm run osm:audit-relations -- <changeset-id> [changeset-id ...]`. The command exits with code `2` when a modified relation changed its ordered `(type, ref, role)` member list.
