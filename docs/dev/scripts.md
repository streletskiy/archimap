...
- `node --import tsx scripts/benchmark-region-sync.ts --region-id=<id>`: run the live region sync pipeline against a real managed region, capture stage timings/peak RSS/PMTiles build metadata, and write a JSON report under `.benchmarks/`. It auto-seeds the region catalog row from `frontend/static/admin-regions.geojson` when needed, and supports `--passes`, `--pmtiles-only`, `--output`, and `--sample-interval-ms`.

Notes:

- `scripts/sync-osm-region.ts` is intentionally thin; implementation stages live in `scripts/region-sync/extract-download.ts`, `osm2pgsql-import.ts`, `region-db.ts`, `import-applier.ts`, and `pmtiles-builder.ts`.
- Managed region sync is PostgreSQL-only. The runtime path no longer uses SQLite, QuackOSM, DuckDB, Python importer helpers, `osmium`, or `tippecanoe`.
- `pmtiles-builder.ts` uses `planetiler` for regional PMTiles builds.
- Managed imports use `osm2pgsql` flex into a per-run PostgreSQL staging schema and then merge into canonical tables; `REGION_SYNC_IMPORT_APPLY_BATCH_SIZE` still controls apply batching details where relevant.
