...
- `npm run admin:regions:pmtiles`: rebuild the admin region coverage archive in `frontend/static/admin-regions.pmtiles` from `frontend/static/admin-regions.geojson`. By default it uses local `planetiler` when available, otherwise a Dockerized runtime-base fallback; `--engine tippecanoe` switches it back to the old path for comparison or rollback. The command also writes `frontend/static/admin-regions.pmtiles.meta.json` with the GeoJSON hash used for freshness checks.
...
- `pmtiles-builder.ts` uses `planetiler` by default for regional PMTiles builds. When `PMTILES_BUILD_ENGINE=tippecanoe`, it switches to the old sharded fallback path: large regions are split into a km-aligned grid (see `REGION_SYNC_SHARD_KM`, default `60`) and merged with `tile-join`, keeping peak tippecanoe memory bounded for country-sized rebuilds on low-RAM SBCs; `REGION_SYNC_SHARD_KM=0` restores the single-pass tippecanoe build. When `REGION_SYNC_SHARD_MIN_FEATURES` is unset, the tippecanoe fallback uses an adaptive floor so small and medium regions can stay on the single-pass path. The tippecanoe path also keeps a persistent shard cache under `data/regions/.pmtiles-cache/` and only rebuilds dirty shards.
