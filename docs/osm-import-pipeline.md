...
- Managed sync workers keep a heartbeat on every owned `queued`/`running` run. Restart recovery archives stale active runs as `abandoned`, and only requeues the affected region when that stale run has not already been superseded by a later successful sync for the same region. If a previously recovered stale run later finishes successfully, any redundant `startup` retry rows for the same region are archived as superseded and can no longer overwrite the successful result. This prevents false `Sync interrupted by process restart` errors while also avoiding redundant restarts after a successful import is already present in the DB and region archive.
