...
- `scripts/region-sync/pmtiles-builder.ts` uses `planetiler` by default. The default path is a single-pass build over the region GeoJSON NDJSON export and does not keep a shard cache.
- When `PMTILES_BUILD_ENGINE=tippecanoe`, the builder switches to the older sharded fallback path: every multi-cell region is split into a square km-grid before running `tippecanoe`, then the per-cell archives are merged with `tile-join`. Each tippecanoe invocation processes only one cell, so peak RSS scales with the _densest_ cell instead of the whole region.
- Cell size is controlled by `REGION_SYNC_SHARD_KM` (default `60`) for the tippecanoe fallback path. The grid is planned from the region bbox using a cosine-adjusted longitude step so cells stay roughly square on the ground at the region's latitude. Set `REGION_SYNC_SHARD_KM=0` to force the single-pass tippecanoe path for the whole deployment.
- Single-cell regions (bbox smaller than one shard cell) automatically collapse to a direct tippecanoe call, so small cities still take the fast path. `REGION_SYNC_SHARD_MIN_FEATURES` defaults to an adaptive floor when unset, which keeps small and medium regions on the single-pass tippecanoe path without hard-coding one universal threshold. Set it explicitly if you need reproducible benchmarking behavior.
- In the sharded tippecanoe path, each feature is assigned to a cell by the bbox center of its geometry, so building footprints and `building:part` geometry never straddle two shards. Invalid or geometry-less lines are counted as `skippedFeatureCount` and reported at the start of the sharded build.
- The tippecanoe fallback keeps a persistent shard cache under `data/regions/.pmtiles-cache/`: it hashes each shard input, reuses matching shard archives on repeat runs, and only rebuilds dirty shards before `tile-join` merges cached and rebuilt inputs. This is the main reason repeat tippecanoe PMTiles rebuilds can be much cheaper than the first run.
...
- The builder accepts `PLANETILER_BIN` as an override path for the default engine, plus `TIPPECANOE_BIN` / `TILE_JOIN_BIN` as override paths for the sharded fallback engine.
