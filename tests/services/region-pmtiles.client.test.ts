...
test('region building layers stay disabled until zoom 13', async () => {
  const { REGION_BUILDING_LAYER_MIN_ZOOM, shouldRenderRegionBuildings } = await loadRegionPmtiles();
...
test('viewport coverage helper works with intersecting region subsets', async () => {
  const { getActiveRegionPmtiles, isViewportCoveredByRegions } = await loadRegionPmtiles();
  ...
  assert.deepEqual(
    activeRegions.map((region) => region.id),
    [1, 2]
  );
