...
const { createPythonExtractResolver, ensurePythonImporterDeps } = require('../../scripts/region-sync/python-extractor');
...
test(
  'searchExtractCandidates returns canonical candidates for free-form query',
  pythonExtractorTestOptions,
  async () => {
...
test(
  'resolveExactExtract accepts path-style osmfr region ids from admin map data',
  pythonExtractorTestOptions,
  async () => {
...
test(
  'resolveExactExtract accepts path-style geofabrik us state ids from admin map data',
  pythonExtractorTestOptions,
  async () => {
...
test(
  'resolveExactExtract reports ambiguous exact-name matches instead of auto-selecting one',
  pythonExtractorTestOptions,
  async () => {
...
test(
  'download_extract_with_progress emits managed stage markers for local HTTP downloads',
  pythonExtractorTestOptions,
  async () => {
    ...
    const pythonScript = [
      ...
      'import pathlib',
      'spec = importlib.util.spec_from_file_location("sync_osm_buildings", r"' +
        importerPath.replace(/\\/g, '\\\\') +
        '")',
    ...
    try {
      const result: { code: number | null; stdout: string; stderr: string } = await new Promise((resolve, reject) => {
        const child = spawn(pythonCandidate.exe, [...pythonCandidate.prefixArgs, '-c', pythonScript], {
