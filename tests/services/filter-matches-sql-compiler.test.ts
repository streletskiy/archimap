...
test('compilePostgresFilterRulePredicate: compiles supported operators with stable params', () => {
  ...
  assert.match(
    contains.sql,
    /strpos\(lower\(CASE WHEN COALESCE\(length\(btrim\(src\.name\)\), 0\) > 0 THEN src\.name ELSE jsonb_extract_path_text\(src\.tags_jsonb, \?\) END\), \?\) > 0/
  );
  ...
  assert.match(
    equals.sql,
    /lower\(CASE WHEN COALESCE\(length\(btrim\(src\.style\)\), 0\) > 0 THEN src\.style ELSE COALESCE\(jsonb_extract_path_text\(src\.tags_jsonb, \?\), jsonb_extract_path_text\(src\.tags_jsonb, \?\), jsonb_extract_path_text\(src\.tags_jsonb, \?\)\) END\) = \?/
  );
  ...
  assert.match(
    colour.sql,
    /lower\(CASE WHEN COALESCE\(length\(btrim\(src\.colour\)\), 0\) > 0 THEN src\.colour ELSE COALESCE\(jsonb_extract_path_text\(src\.tags_jsonb, \?\), jsonb_extract_path_text\(src\.tags_jsonb, \?\)\) END\) = \?/
  );
  ...
  assert.match(
    notEquals.sql,
    /lower\(CASE WHEN COALESCE\(length\(btrim\(jsonb_extract_path_text\(src\.tags_jsonb, \?\)\)\), 0\) > 0 THEN jsonb_extract_path_text\(src\.tags_jsonb, \?\) ELSE NULL::text END\) <> \?/
  );
  ...
  assert.match(
    startsWith.sql,
    /lower\(CASE WHEN COALESCE\(length\(btrim\(jsonb_extract_path_text\(src\.tags_jsonb, \?\)\)\), 0\) > 0 THEN jsonb_extract_path_text\(src\.tags_jsonb, \?\) ELSE NULL::text END\) LIKE \(\? \|\| '%'\)/
  );
  ...
  assert.match(
    exists.sql,
    /COALESCE\(length\(btrim\(CASE WHEN COALESCE\(length\(btrim\(src\.style\)\), 0\) > 0 THEN src\.style ELSE COALESCE\(jsonb_extract_path_text\(src\.tags_jsonb, \?\), jsonb_extract_path_text\(src\.tags_jsonb, \?\), jsonb_extract_path_text\(src\.tags_jsonb, \?\)\) END\)\), 0\) > 0/
  );
  ...
  assert.match(
    notExists.sql,
    /COALESCE\(length\(btrim\(CASE WHEN COALESCE\(length\(btrim\(src\.name\)\), 0\) > 0 THEN src\.name ELSE jsonb_extract_path_text\(src\.tags_jsonb, \?\) END\)\), 0\) = 0/
  );
  ...
  assert.match(
    designRef.sql,
    /lower\(CASE WHEN COALESCE\(length\(btrim\(src\.design_ref\)\), 0\) > 0 THEN src\.design_ref ELSE jsonb_extract_path_text\(src\.tags_jsonb, \?\) END\) = \?/
  );
...
test('compilePostgresFilterRulesGuardPredicate: combines guard predicates and preserves params order', () => {
  ...
  assert.deepEqual(compiled.params, ['foo', 'bar']);
});
    ...
    [
      {
        ...
        rulesHash: 'hash-1',
        maxResults: 100,
        rules: [{ key: 'building:levels', op: 'equals', value: '1', valueNormalized: '1', numericValue: null }]
      },
      {
        id: 'levels-3-8',
        rulesHash: 'hash-2',
  ...
  const styleResult = buildFilterMatchBatchResults(items, [
    {
      ...
      maxResults: 100,
      rules: [{ key: 'style', op: 'equals', value: 'Local Style', valueNormalized: 'local style', numericValue: null }]
    }
  ]);
  ...
  const materialResult = buildFilterMatchBatchResults(items, [
    {
      ...
      maxResults: 100,
      rules: [{ key: 'material', op: 'equals', value: 'stone', valueNormalized: 'stone', numericValue: null }]
    }
  ]);
  ...
  const colourResult = buildFilterMatchBatchResults(items, [
    {
      ...
      maxResults: 100,
      rules: [{ key: 'colour', op: 'equals', value: 'red', valueNormalized: 'red', numericValue: null }]
    }
  ]);
...
test('buildFilterMatchBatchResults returns actual marker points at and above the low-zoom aggregation cutoff', () => {
  ...
  assert.deepEqual(
    results[0].matchedLocations.map((point) => point?.count ?? null),
    [1, 1, 1, 1]
  );
  assert.equal(
    results[0].matchedLocations.every((point) => String(point?.osmKey || '').startsWith('way/')),
    true
  );
  assert.equal(
    results[0].matchedLocations.every((point) => String(point?.osmKey || '').startsWith('cell:') === false),
    true
  );
