...
test('escapePostgresCopyTextValue formats nulls and control characters for COPY text', () => {
  ...
  assert.equal(escapePostgresCopyTextValue('one\\two\tthree\nfour\rfive'), 'one\\\\two\\tthree\\nfour\\rfive');
});
...
test('applyRegionImport batches sqlite imports and keeps only ids in temp cleanup state', async () => {
  ...
  try {
    ...
    const membershipsByRegion = verifyArchimapDb
      .prepare(
        `
    ...
    `
      ...
      .all();
    const removedRegionMembership = verifyArchimapDb
      .prepare(
        `
    ...
    const contourCount = verifyOsmDb
      .prepare(
        `
      SELECT COUNT(*) AS total
    ...
    `
      ...
      .get();
    const updatedContour = verifyOsmDb
      .prepare(
        `
    ...
    `
      ...
      .get();
    const removedContour = verifyOsmDb
      .prepare(
        `
      SELECT COUNT(*) AS total
      FROM building_contours
    ...
    `
      ...
      .get();
    const sharedContour = verifyOsmDb
      .prepare(
        `
      SELECT tags_json
      FROM building_contours
