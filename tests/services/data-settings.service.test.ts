...
const migration = require('../../db/migrations/003_data_regions.migration.ts');
...
function createMockExtractResolver(fixtures: LooseRecord = {}) {
  ...
  return {
    async searchExtractCandidates(query, options: LooseRecord = {}) {
    ...
    async resolveExactExtract(query, options: LooseRecord = {}) {
...
function createService(options: LooseRecord = {}) {
...
function buildRegionInput(input: LooseRecord = {}) {
...
test('filter preset defaults are bootstrapped for admin/runtime payloads', async () => {
  ...
  for (const item of adminPresets.items) {
