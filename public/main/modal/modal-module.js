    ...
    const editableRows = ctx.canEditBuildings
      ...
      <form id="building-edit-form" class="grid gap-2">
        <div class="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 text-slate-700 shadow-soft">
          ...
          <input id="building-name" name="building-name" type="text" value="${escapeHtml(info.name || (ctx.shownName !== '-' ? ctx.shownName : ''))}" class="ui-field mt-1.5 w-full bg-white" />
        </div>
        <div class="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 text-slate-700 shadow-soft">
          ...
          <input id="building-levels" name="building-levels" type="number" value="${escapeHtml(info.levels ?? (ctx.shownLevels !== '-' ? ctx.shownLevels : ''))}" class="ui-field mt-1.5 w-full bg-white" />
        </div>
        <div class="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 text-slate-700 shadow-soft">
          ...
          <input id="building-year" name="building-year" type="number" value="${escapeHtml(info.year_built || (ctx.shownYear !== '-' ? ctx.shownYear : ''))}" class="ui-field mt-1.5 w-full bg-white" />
        </div>
        <div class="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 text-slate-700 shadow-soft">
          ...
          <input id="building-architect" name="building-architect" type="text" value="${escapeHtml(info.architect || (ctx.shownArchitect !== '-' ? ctx.shownArchitect : ''))}" class="ui-field mt-1.5 w-full bg-white" />
        </div>
        <div class="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 text-slate-700 shadow-soft">
          ...
          <select id="building-style-select" name="building-style-select" class="ui-field mt-1.5 w-full bg-white">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 text-slate-700 shadow-soft">
          ...
          <textarea id="building-archimap-description" name="building-archimap-description" rows="3" class="ui-field mt-1.5 w-full bg-white">${escapeHtml(info.archimap_description || info.description || '')}</textarea>
        </div>
        <div class="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 text-slate-700 shadow-soft">
          ...
          <div class="mt-1.5 grid gap-1.5 md:grid-cols-2">
            <label class="block md:col-span-2"><span class="mb-1 block text-xs font-semibold text-slate-700">${escapeHtml(t('modalAddressFull', null, 'Полный адрес (addr:full)'))}</span><input id="building-addr-full" name="building-addr-full" type="text" value="${escapeHtml(ctx.addressForm.full)}" class="ui-field w-full bg-white" /></label>
            <label class="block"><span class="mb-1 block text-xs font-semibold text-slate-700">${escapeHtml(t('modalAddressPostcode', null, 'Индекс (addr:postcode)'))}</span><input id="building-addr-postcode" name="building-addr-postcode" type="text" value="${escapeHtml(ctx.addressForm.postcode)}" class="ui-field w-full bg-white" /></label>
            <label class="block"><span class="mb-1 block text-xs font-semibold text-slate-700">${escapeHtml(t('modalAddressCity', null, 'Город (addr:city)'))}</span><input id="building-addr-city" name="building-addr-city" type="text" value="${escapeHtml(ctx.addressForm.city)}" class="ui-field w-full bg-white" /></label>
            <label class="block"><span class="mb-1 block text-xs font-semibold text-slate-700">${escapeHtml(t('modalAddressPlace', null, 'Место/локация (addr:place)'))}</span><input id="building-addr-place" name="building-addr-place" type="text" value="${escapeHtml(ctx.addressForm.place)}" class="ui-field w-full bg-white" /></label>
            <label class="block"><span class="mb-1 block text-xs font-semibold text-slate-700">${escapeHtml(t('modalAddressStreet', null, 'Улица (addr:street)'))}</span><input id="building-addr-street" name="building-addr-street" type="text" value="${escapeHtml(ctx.addressForm.street)}" class="ui-field w-full bg-white" /></label>
            <label class="block md:col-span-2"><span class="mb-1 block text-xs font-semibold text-slate-700">${escapeHtml(t('modalAddressHouseNumber', null, 'Номер дома (addr:housenumber)'))}</span><input id="building-addr-housenumber" name="building-addr-housenumber" type="text" value="${escapeHtml(ctx.addressForm.housenumber)}" class="ui-field w-full bg-white" /></label>
          </div>
        </div>
        <div class="flex items-center justify-between gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2.5"><p id="building-save-status" class="text-sm text-slate-600"></p><button type="submit" class="ui-btn ui-btn-primary">${escapeHtml(t('modalSave', null, 'Сохранить'))}</button></div>
      </form>
    `
      : `
