...
async function main() {
  ...
  try {
    ...
    const setup = await page.evaluate(
      async ({ camera, featureCount, color, samples, warmups }) => {
        ...
        map.addLayer({
          ...
          paint: {
            ...
            'fill-opacity': ['case', ['boolean', ['feature-state', 'isFiltered'], false], 0.4, 0]
          }
        });
        map.addLayer({
          id: FS_LINE_ID,
          ...
          paint: {
            ...
            'line-width': ['case', ['boolean', ['feature-state', 'isFiltered'], false], 1.8, 0],
            'line-opacity': ['case', ['boolean', ['feature-state', 'isFiltered'], false], 0.95, 0]
          }
        });
        ...
        async function clearFeatureState() {
          for (const id of ids) {
            map.setFeatureState({ source: SOURCE_ID, id }, { isFiltered: false, filterColor: '#000000' });
          }
          await waitFrames(3);
        }
        ...
        async function applyFeatureStateChunked() {
          ...
          while (index < ids.length) {
            ...
            while (
              ...
              map.setFeatureState({ source: SOURCE_ID, id: ids[index] }, { isFiltered: true, filterColor: color });
              index += 1;
              frameOps += 1;
            }
            if (index < ids.length) {
        ...
        async function benchmarkModes(modes) {
          const samplesByName = new Map<string, number[]>(modes.map((mode) => [mode.name, [] as number[]]));
          ...
          for (let sampleIndex = 0; sampleIndex < samples; sampleIndex += 1) {
            for (const mode of getRoundRobinModes(modes, sampleIndex + warmups)) {
              ...
              await mode.apply();
              const sampleValues = samplesByName.get(mode.name);
              if (sampleValues) {
                sampleValues.push(performance.now() - startedAt);
              }
            }
          }
