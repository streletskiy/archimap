...
function printLegacyModeError() {
  const help = [
    ...
    `  node ${path.join('scripts', 'sync-osm-region.js')} --region-id=<id>`
  ];
...
function main() {
  ...
  const regionSyncScriptPath = path.join(__dirname, 'sync-osm-region.js');
  const delegated = spawnSync(process.execPath, [regionSyncScriptPath, ...args], {
    stdio: 'inherit',
    shell: false,
    env: process.env
  });
