...
function createStringMatcher(pattern, { prefix = false }: LooseRecord = {}) {
...
function toMatchers(pattern: string | RegExp | readonly (string | RegExp)[], options: LooseRecord = {}) {
...
function createMiniApp() {
  ...
  app.use = (...args) => {
    let pattern: string | RegExp | readonly (string | RegExp)[] = '/';
    let handlers = args;
