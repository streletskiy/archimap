function createAuthMiddlewareSupport(options: LooseRecord = {}) {
  const { db, getUserEditRequiresPermission = () => true } = options;
  ...
  async function requireAuth(req, res, next) {
    ...
    const row = await db
      .prepare(
        'SELECT email, can_edit, is_admin, is_master_admin, first_name, last_name FROM auth.users WHERE email = ?'
      )
      .get(email);
    if (!row) {
      return res.status(401).json({ code: 'ERR_AUTH_REQUIRED', error: 'Authentication is required' });
  ...
  async function requireBuildingEditPermission(req, res, next) {
    ...
    if (!email) {
      return res
        .status(403)
        .json({ code: 'ERR_EDITING_UNAVAILABLE', error: 'Editing is unavailable for this account' });
    }
    ...
    if (Number(row.can_edit || 0) <= 0) {
      return res
        .status(403)
        .json({
          code: 'ERR_EDITING_FORBIDDEN',
          error: 'Editing is not allowed for this account. Contact an administrator for access.'
        });
    }
