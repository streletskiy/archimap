...
const { buildFilterMatchBatchResults, createBuildingFiltersService } = require('../services/building-filters.service');
...
function buildSourceSnapshot(options: any = {}) {
  ...
  return {
    ...
    sourceOsmUpdatedAt:
      toIsoTimestampOrNull(body?.sourceOsmUpdatedAt) || toIsoTimestampOrNull(fallbackRow?.source_osm_updated_at)
  };
}
...
function registerBuildingsRoutes(deps) {
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      if (!sourceSnapshot.sourceGeometryJson || !sourceSnapshot.sourceTagsJson) {
        return res
          .status(404)
          .json({ code: 'ERR_BUILDING_NOT_FOUND', error: 'Building source snapshot was not found' });
      }
      ...
      if (featureKind === 'building_part') {
        ...
        const hasDisallowedPayloadFields = [
          'name',
          'design',
          'design_ref',
          'design_year',
  ...
  app.get('/api/building/:osmType/:osmId', buildingsReadRateLimiter, async (req, res) => {
    ...
    if (!row) {
      return res
        .status(404)
        .json({ code: 'ERR_BUILDING_NOT_FOUND', error: 'Building was not found in the local contours database' });
    }
