...
function createOsmSyncService(options = {}) {
  ...
  function parseGeneralSettingsRow(row) {
    return {
  ...
  function normalizeStoredSettings(row, generalBaseUrl = '') {
  ...
  async function saveSettings(input = {}, actor = null) {
  ...
  async function fetchText(url, init = {}) {
    ...
    if (!response.ok) {
      const error = new Error(text || `OSM request failed (${response.status})`);
  ...
  async function exchangeCodeForToken({ code, verifier, authBaseUrl, redirectUri, clientId, clientSecret }) {
    ...
    if (!response.ok) {
      const error = new Error(String(payload?.error_description || payload?.error || text || `OSM token exchange failed (${response.status})`));
      error.status = response.status;
  ...
  async function storeTokenBundle(bundle, actor = null) {
    const current = await readSettingsRow();
    const general = await readGeneralSettings();
  ...
  async function handleOauthCallback({ code, state }) {
    ...
    if (!code) {
      const error = new Error('OAuth code is missing');
      error.status = 400;
  ...
  function stateFromLocalRow(row = {}) {
    return {
  ...
  function stateFromContourTags(tags = {}) {
    return {
  ...
  function applyFieldToTagMap(tags, field, value, explicitlyEdited = false) {
  ...
  function buildDesiredTagMap(currentTags, candidateRows) {
    ...
    return { desired, localState, explicitFields: [...explicitFields], removedKeys: [...removedKeys] };
  }
  ...
  function sanitizeElementAttrs(attrs, elementType, changesetId) {
  ...
  async function updateOsmElement(accessToken, apiBaseUrl, currentElement, desiredTags, changesetId) {
    const attrs = sanitizeElementAttrs(currentElement.attrs, currentElement.type, changesetId);
    const bodyXml = buildElementXml({
      type: currentElement.type,
      attrs,
      beforeTags: currentElement.beforeTags
    }, desiredTags);
    return fetchText(new URL(`/api/0.6/${encodeURIComponent(currentElement.type)}/${encodeURIComponent(currentElement.attrs.id)}`, apiBaseUrl), {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${accessToken}`,
        'Content-Type': 'text/xml; charset=utf-8',
  ...
  function buildCandidateRecord(rows) {
  ...
  async function updateSyncRows(osmType, osmId, patch, statuses = ['accepted', 'partially_accepted']) {
  ...
  async function prepareSyncCandidateSyncData(osmType, osmId, candidate = null) {
  ...
  async function syncCandidatesToOsm(targets, _actor = null) {
    ...
    if (normalizedTargets.length === 0) {
      const error = new Error('No sync candidates were provided');
      error.status = 400;
    ...
    const creds = await getCredentials();
    if (!creds.accessToken) {
      const error = new Error('OSM access token is not configured');
      error.status = 503;
    ...
    for (const target of normalizedTargets) {
      ...
      if (!candidate) {
        const error = new Error('Sync candidate was not found');
        error.status = 404;
      ...
      if (candidate.syncReadOnly) {
        const error = new Error('This building group has already been synchronized and is now read-only');
        error.status = 409;
      ...
      if (candidate.syncStatus === 'syncing') {
        const error = new Error('This building group is currently being synchronized');
        error.status = 409;
      ...
      if (!candidate.canSync) {
        const error = new Error('No accepted local edits are available for sync');
        error.status = 409;
      ...
      if (!candidate.liveElement) {
        const error = new Error(candidate.preflightError || 'Unable to load the current OSM element');
        error.status = 404;
      ...
      if (!candidate.sourceMatches) {
        const error = new Error('OSM element changed since the source snapshot was captured');
        error.status = 409;
