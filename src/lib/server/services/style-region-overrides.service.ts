type LooseStyleOverrideError = Error & {
  status?: number;
  code?: string;
};

function createStyleRegionOverrideError(status, message, options: LooseRecord = {}) {
  const error = new Error(String(message || 'Style region override request failed')) as LooseStyleOverrideError;
...
function mapOverrideRow(row: LooseRecord, { publicOnly = false }: LooseRecord = {}) {
...
function createStyleRegionOverridesService(options: LooseRecord = {}) {
  const { db } = options;
  ...
  async function saveOverride(input: LooseRecord = {}, actor = null) {
    const normalized = {
