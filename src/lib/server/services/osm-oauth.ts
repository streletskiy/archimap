...
function createOsmOauthController(deps: OauthDeps) {
  ...
  async function saveSettings(input: LooseRecord = {}, actor = null) {
    ...
    const authBaseUrl = normalizeAuthBaseUrl(
      input.authBaseUrl || currentNormalized.authBaseUrl || DEFAULT_AUTH_BASE_URL
    );
    ...
    const redirectUri = normalizeRedirectUri(
      input.redirectUri || currentNormalized.redirectUri || '',
      general.appBaseUrl || ''
    );
    const clientId = normalizeText(input.clientId, 160);
    const providerName = normalizeText(input.providerName, 80) || 'OpenStreetMap';
    const clientSecretInput = Object.prototype.hasOwnProperty.call(input, 'clientSecret')
      ? String(input.clientSecret || '').trim()
      : null;
    const nextClientSecretEnc =
      clientSecretInput != null
        ? clientSecretInput
          ? encrypt(clientSecretInput)
          : null
