...
function buildSmtpDeliveryCandidates(raw = {}) {
...
async function sendMailWithFallback(rawSmtpConfig, mailOptions = {}, options = {}) {
  ...
  for (let index = 0; index < candidates.length; index += 1) {
    ...
    try {
      ...
      if (accepted.length === 0) {
        const err = new Error('SMTP accepted no recipients');
        err.code = 'EENVELOPE';
        err.command = 'RCPT TO';
