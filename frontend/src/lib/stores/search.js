...
export function setSearchLoading({ append = false, background = false } = {}) {
  searchState.update((state) => ({
    ...state,
...
export function setSearchMapLoading({ query, bboxHash, preserveItems = true } = {}) {
...
export function applySearchMapResults({ query, bboxHash, items, total, truncated = false }) {
...
export function setSearchMapError(message, { preserveItems = true } = {}) {
  const text = String(message || translateNow('search.failed'));
  searchMapState.update((state) => ({
    ...state,
    loading: false,
