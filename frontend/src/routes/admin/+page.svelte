<script>
  ...
  async function switchTab(tab, options = {}) {
    ...
    if ((tab === 'data' || tab === 'filters') && $session.user?.isMasterAdmin) {
  ...
  onMount(() => {
    ...
    if ($session.authenticated && $session.user?.isAdmin) {
      ...
      } else if ((activeTab === 'data' || activeTab === 'filters') && $session.user?.isMasterAdmin) {
        loadDataSettings({ preserveSelection: true });
      }
    }
        ...
        <section class="mt-3 space-y-4 rounded-2xl border ui-border ui-surface-base p-4 min-w-0">
          ...
          <div class="flex flex-wrap gap-2 text-xs ui-text-subtle">
            <span class="rounded-full ui-surface-soft px-2 py-1">
              {$t('admin.data.filterTags.source')}: {dataSettings.filterTags.source}
            </span>
            <span class="rounded-full ui-surface-soft px-2 py-1">
